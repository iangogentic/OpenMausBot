...
const COMPUTER_OPERATOR_PROVIDER = createComputerOperatorProviderRuntime({
  prepare: async (input) => {
    ...
    return {
      ...
      turn: {
        // Reuse one durable Hermes HOME per bot + target instead of allocating
        // one for every child id (the supervisor deliberately caps persistent
        // homes). Distinct computers can still run concurrently without
