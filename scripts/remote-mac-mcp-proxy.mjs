...
if (host !== "127.0.0.1" || !Number.isInteger(port) || port < 1024 || port > 65535 || !tokenFile) {
  process.stderr.write("Invalid remote Mac bridge configuration\n");
  process.exit(64);
}
...
try {
  ...
  process.stderr.write("Remote Mac bridge token is unavailable or unsafe\n");
  process.exit(78);
}
...
socket.setTimeout(10_000, () => fail("Remote Mac bridge timed out"));
socket.once("connect", () => socket.write(`OMB-MAC-BRIDGE/1 ${token}\n`));
socket.on("data", function receive(chunk) {
  ...
  if (newline < 0) {
    if (handshake.length > 64) fail("Invalid remote Mac bridge handshake");
    return;
  }
  const response = handshake.subarray(0, newline).toString("utf8").replace(/\r$/, "");
  if (response !== "OK") {
    fail(response === "DENIED" ? "Physical Mac access was denied" : "Physical Mac is unavailable", 77);
    return;
  }
  ready = true;
  socket.setTimeout(0);
...
socket.once("error", (error) => fail(`Remote Mac bridge connection failed: ${error.message}`));
socket.once("close", () => {
  if (!ready && process.exitCode === undefined) fail("Remote Mac bridge closed before authentication");
  process.stdin.pause();
});
