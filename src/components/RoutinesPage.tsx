        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-1.5 truncate text-[10.5px] text-white/70">
            ...
            <span className="truncate">
              ...
              {(item.routine?.runOn ?? item.run?.runOn) === "cloud" ? " · Box" : ""}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Laptop size={15} />Bot's current setup</div>
              ...
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Cloud size={15} />Hosted Box</div>
            ...
            {runOn === "cloud" && (
              <div className={cn("mt-2 rounded-lg px-3 py-2 text-[11.5px] leading-relaxed", cloudReady ? "bg-accent/10 text-ink-secondary" : "border border-warning/25 bg-warning/10 text-warning")}>
                {cloudReady
                  ? "The Hosted Box wakes automatically for each run. Keep OpenMausBot running so its scheduler can launch the job."
                  : "Hosted Box needs a working Box API key in App Settings before this routine can run."}
              </div>
            )}
          </div>
          <div>
        ...
        <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
          {routine && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{routine.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{routine.runOn === "cloud" ? "Hosted Box" : "MAUS setup"}</div></div>
          ...
          {run?.triggerSource === "webhook" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{run.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{run.runOn === "cloud" ? "Hosted Box" : "MAUS setup"}</div></div>
